class Planet {
  constructor(coor, numFabricas) {
    ...
    this.node.addEventListener("click", () => {
      if (!this.returnIsBussy()) {
        ...
        console.log("espera a que se ejecute la accion en el planeta.");
      }
    });
  }
  ...
  fabricarEjercito = () => {
    if (this.tropas <= this.maxTropas) this.tropas += 0.1 * this.numFabricas;
  };
  ...
  actualizarTextoInterno = () => {
    ...
    if (this.owner === "neutral") {
      this.node.style.borderColor = "rgb(35, 238, 133)";
    }
    if (this.owner === "enemy") {
